...
#errors_plot = errors_plot[np.logical_not(np.isnan(errors_plot))]
...
if my_kernel == 'kernel1':
    ...
    plt.title(r'Relative error, $k=C_1 \exp(-x^2/\ell^2)+ \epsilon_{WK}$')
elif my_kernel == 'kernel6':
    plt.title(r'Relative error, $k=C_1 \exp(-x^2/\ell^2)+ C_2$')
elif my_kernel == 'kernel7':
    plt.title(r'Relative error, $k=C_1 \exp(-x^2/\ell^2)+ C_2 \epsilon_{WK}$')
elif my_kernel == 'kernel_ell':
    plt.title(r'Relative error, $k=C_1 \exp(-(x^T M x))+ C_2$')
elif my_kernel == 'kernel_rq':
    plt.title(r'Relative error, $k=C_1 \exp(-(x^T M x)) + C_2  \Big( 1+ \frac{r^2}{2 \alpha \ell^2} \Big)^{- \alpha}$')
...
f = plt.figure(1)
plt.plot(msq_sort[0::1], y_pred_sort_msq[0::1], 'o')
plt.plot(msq_sort[0::1], y_test_sort_msq[0::1], alpha=0.7)
...
g = plt.figure(2)
plt.plot(mg_sort[0::1], y_pred_sort_mg[0::1], 'o')
plt.plot(mg_sort[0::1], y_test_sort_mg[0::1], alpha=0.7)
plt.title('Cross section (true and predicted)')
