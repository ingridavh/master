...
data_lin = pd.read_csv('data/pears.dat', sep=' ', skipinitialspace=True)
...
kernel5 = C(1.0, (1e-2, 1e2)) * RBF(10, (1e-3, 1e3)) +  WhiteKernel(noise_level=0.1, noise_level_bounds=(1e-10, 1e+3)) 


# Set the wanted kernel
if my_kernel == 'kernel1':
    my_kernel = kernel1
elif my_kernel == 'kernel2':
    my_kernel = kernel2
...
print "My kernel is ", my_kernel 

# Free errors through white noise

print "Fitting the model to the data... \n"
gp = GaussianProcessRegressor(kernel=my_kernel, alpha=0.01, normalize_y=False, n_restarts_optimizer=5).fit(features, target)
