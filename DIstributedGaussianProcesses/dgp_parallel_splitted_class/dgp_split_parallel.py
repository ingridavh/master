...
class dgp_split_parallel:
    ...
    def predict(self, X_test, y_test, log_true=True, alpha=0.001):
        ...
        for i in range(n_experts):
            ...
            #prior_cov = out[i].kernel_(X_test)
            #prior_cov = np.diag(self.kernel(X_test))
            
            mu_star = prediction[i][0]
            sigma_star_mean = prediction[i][1]

            sigma_rbcm_neg_fill = self.fill_my_sigmas(N, mu_star, sigma_star_mean, X_test)
        ...
        for i in range(n_experts):
            ...
            mu_rbcm_fill = self.fill_my_mus(N, mu_star, sigma_star_mean, X_test, sigma_rbcm_neg)
    ...
    def fill_my_sigmas(self, N, mus, sigmas, X_test):
        #########################################################################
    ...
    def fill_my_mus(self, N, mus, sigmas, X_test, sigma_rbcm_neg):
        #########################################################################
        # Do distributed Gaussian process to estimate the value of              #
