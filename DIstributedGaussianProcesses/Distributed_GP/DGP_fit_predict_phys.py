...
from dgp_split_parallel import dgp_split_parallel
import sys
import numpy as np
import sklearn as sk
from sklearn.gaussian_process.kernels import RationalQuadratic, RBF, WhiteKernel, ExpSineSquared, DotProduct, ConstantKernel as C
...
X_train, X_test, y_train, y_test = train_test_split(features, target, random_state=None, train_size = trainsize)

 
my_dgp = dgp_split_parallel(n_experts, outfile, kernel=my_kernel, verbose=False, njobs=my_njobs)
print "Managed to create an instance ", my_dgp
my_dgp.fit(X_train, y_train, alpha=0.01)
mu, sigma = my_dgp.predict(X_test, y_test,  alpha=0.01)
