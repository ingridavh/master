...
class dgp_parallel:
    ...
    def fit_and_predict(self, X, y, trainsize=0.1, log_true=True, alpha=0.001):
        ...
        for i in range(n_experts):
            sigma_rbcm_neg_fill = self.fill_my_sigmas(out[i], X_test, mus[i], sigmas[i], priors[i])
            sigma_rbcm_neg += sigma_rbcm_neg_fill
        
        # Fill in mu values using the sum of sigma
        for i in range(n_experts):
            mu_rbcm_fill = self.fill_my_mus(out[i], X_test, mus[i], sigmas[i], priors[i], sigma_rbcm_neg)
